...
export function Diagnostics({ data, onRefresh, onUpdate }: {
  ...
  if (data.savestate_search) {
    ...
    rows.push(["Save folders searched", data.savestate_search.directories.map(
      (entry) => `${entry.path} — ${entry.status}`,
  ...
  if (hooks) {
    ...
    if (hooks.installed_hooks > 0 && hooks.activation_config) {
      ...
      if (hooks.activation_config.path) rows.push(["ES-DE settings file", hooks.activation_config.path]);
    }
    if (hooks.last_event) {
